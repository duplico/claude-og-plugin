...
- **`claude-tmux`** -- manage a persistent tmux session of Claude Code windows. `new` / `save` / `restore` / `status` / `install-hooks`. Snapshots which Claude session is in which window to `~/.claude/tmux-state.json`, so a reboot (or a closed terminal) does not cost you your running agents. Requires `tmux` and `jq`.
